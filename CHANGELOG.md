...
- The `artifact_then_dispatch` workflow template now also has focused coverage for the combined human-review case, proving `budget_policy_json.human_review_role` can insert `step_human_review` ahead of artifact persistence and still resume later into approval-backed `connector.dispatch`.
