...
OPENVOICE_TTS_PLUGIN_ID = "openvoice_local"  # Legacy config value; never offered as a TTS provider.
...
_OPENVOICE_VOICE_ID_ENV = "OPENVOICE_VOICE_ID"
...
def openvoice_voice_id() -> str:
    return str(os.environ.get(_OPENVOICE_VOICE_ID_ENV) or "").strip()
...
def unmixr_language(default: str = "de") -> str:
    ...
    # identifiers (for example ``de``), while Voice UI/config values are
...
def voicewave_plugin_option(*, configured_voice_id: str, voice_profile_ready: bool) -> dict[str, object]:
    ...
    voice_label = configured_voice_id or voicewave_voice_label()
    if not login_ready:
        ...
        description = "VoiceWave-Studio-Clone fuer voicee Sprachausgabe ist verbunden."
    return {
...
def voicewave_synthesize_request(*, text: str, voice_label: str) -> tuple[bytes, str]:
    ...
    normalized_label = str(voice_label or "").strip() or voicewave_voice_label()
    ...
    *,
    ...
    try:
        response = _unmixr_request(
            ...
            data={
                "name": voice_label,
                "description": f"{slug} voice clone",
            },
            files=[
                (
                    "audio",
                    (
                        f"voice-{slug}{upload_path.suffix or '.wav'}",
