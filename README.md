...
- `GET /v1/human/tasks*` and `GET /v1/human/tasks/backlog` now also accept `sort=last_transition_desc` for freshest ownership churn and `sort=sla_due_at_asc` for earliest pending SLA instead of repository/default order
