...
`scripts/ea_live_ops.py probe-operator-readiness --format operator` or `make probe-operator-readiness` is the first live-ops triage command: it aggregates Telegram delivery, WhatsApp action-processor/pairing posture, Teable recovery, and proactive OODA route/artifact readiness into one no-secret status line with concrete next actions.
...
It also includes `make probe-operator-readiness` as the aggregate live-ops triage entrypoint plus the standalone WhatsApp Web action-processor readiness check via `make verify-whatsapp-web-action-processor-readiness`, so runtime health can be distinguished from live EPUB delivery evidence. If the only remaining blocker is QR/session pairing, `make probe-whatsapp-pairing` writes the current QR SVG to ignored `.runtime/whatsapp-pairing/` and reports the local pairing URL without exposing raw QR data; `make send-whatsapp-pairing-telegram` explicitly sends that QR document over the configured Telegram operator route. For the published link itself, `make verify-whatsapp-audiobook-public-share-playback` proves the shared audiobook route still plays audio in a real browser session.
