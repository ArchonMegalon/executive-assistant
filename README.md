...
- The overlay uses `ea-docker-socket-proxy` at `DOCKER_HOST=tcp://ea-docker-socket-proxy:2375` instead of handing the raw host socket to the API. It constrains the sidecar itself with dropped capabilities, `no-new-privileges`, a read-only rootfs, and bounded memory/PID limits; runs the operator image as its default non-root user; keeps the Docker socket mounted read-only only on the operator image/profile; drops all ambient Linux capabilities from the API; uses the exact Compose inputs baked into the attested runtime image; mounts only `vpn/fastestvpn/` from the host; and binds the host proxy to loopback.
- Fetch reviewed Switzerland profiles with [bootstrap_fastestvpn_configs.sh](scripts/bootstrap_fastestvpn_configs.sh), then use `scripts/rotate_fastestvpn_proxy.sh` to change the bounded sidecar exit without rebuilding the EA runtime.
