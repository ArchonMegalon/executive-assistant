...
- `/v1/skills` now projects a first-class skill catalog layer over task contracts, so executive capabilities like `meeting_prep`, `ltd_inventory_refresh`, the BrowserAct-backed `browseract_bootstrap_manager`, the repair-focused `browseract_workflow_repair_manager`, and the Gemini Vortex-backed `chummer6_visual_director` can persist product-facing metadata (`memory_reads`, `memory_writes`, authority/tool/human/provider policy, evaluation cases, and workflow template selection) without introducing a second storage system; `chummer6_visual_director` now records a reviewed style epoch plus scene-ledger facts so the art direction can remember what it already rendered; `SKILLS.md` tracks the current catalog, and `GET /v1/skills?provider_hint=BrowserAct` filters the catalog by LTD-backed provider hints
- Chummer6 text generation now routes through EA + Gemini Vortex only. If that lane is unavailable, the worker fails instead of quietly falling back to Codex.
