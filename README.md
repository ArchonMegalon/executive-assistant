...
Live memorial promotion is a separate, fail-closed operation. It requires the exact current root-owned terminal qualification permit, a passing isolated candidate receipt, and `make verify-ea-memorial-scoped-deploy`. The standalone ingress reconciler remains verification-only until a reviewed joint API-and-ingress coordinator can own atomic mutation and rollback under the same permit. After an authorized scoped deploy, prove the exact credential-free public memorial and 3D-tour bytes with `make verify-ea-public-ingress-public`, then run `make memorial-gold-gates`. No source or local gate authorizes production mutation by itself.
