...
- `/v1/skills*` promotes those task contracts into product-facing executive skills with explicit workflow, memory, authority, provider-hint, human-policy, and evaluation metadata, including the Gemini Vortex-backed `chummer6_visual_director` lane for Chummer6 guide planning and the BrowserAct-backed `browseract_bootstrap_manager` lane for on-demand workflow-spec generation
...
- `/v1/skills` now projects a first-class skill catalog layer over task contracts, so executive capabilities like `meeting_prep`, `ltd_inventory_refresh`, the BrowserAct-backed `browseract_bootstrap_manager`, and the Gemini Vortex-backed `chummer6_visual_director` can persist product-facing metadata (`memory_reads`, `memory_writes`, authority/tool/human/provider policy, evaluation cases, and workflow template selection) without introducing a second storage system; `SKILLS.md` tracks the current catalog, and `GET /v1/skills?provider_hint=BrowserAct` filters the catalog by LTD-backed provider hints
