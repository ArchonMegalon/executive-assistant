...
| GET | `/v1/rewrite/sessions/{session_id}` | `200` | `404 session not found`, `403 principal_scope_mismatch` (returns events + steps + queue items + receipts + artifacts + costs + human task packets, inline human task assignment history, self-describing artifact/proof task identity, inline assignment-history task identity, `plan_compiled` event, computed reviewer routing hints, and supports `human_task_assignment_source`) |
