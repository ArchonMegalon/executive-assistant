...
- FastestVPN can back the bounded 1min refresh lane through [docker-compose.fastestvpn.yml](docker-compose.fastestvpn.yml): place FastestVPN OpenVPN profiles under `vpn/fastestvpn/`, then deploy with `EA_ENABLE_FASTESTVPN=1 bash scripts/deploy.sh`. The overlay starts only `ea-fastestvpn-proxy-ch`, binds its host port to `127.0.0.1:9315`, and gives only `ea-api` the in-network proxy address. It does not proxy worker, scheduler, WhatsApp, browser UI, or public ingress, and API startup has no VPN health dependency. Check the deployed boundary with `.venv/bin/python scripts/ea_live_ops.py probe-fastestvpn-transport --format operator`. Resume a private live-ops receipt with `EA_ONEMIN_DIRECT_API_PROXY_SERVER=http://127.0.0.1:9315 EA_ONEMIN_DIRECT_API_PROXY_POOL=http://127.0.0.1:9315 .venv/bin/python scripts/ea_live_ops.py refresh-onemin-direct-api --proxy-mode configured --output-json <private-receipt>`. The receipt records only proxy family, configured/reachable counts, country result, and a no-secret assertion. Use `scripts/rotate_fastestvpn_proxy.sh` to recreate the CH sidecar before a broad refresh.
