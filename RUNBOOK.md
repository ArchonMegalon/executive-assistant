...
`EA_TRUST_AUTHENTICATED_PRINCIPAL_HEADER=1` allows authenticated principal-header override only from loopback-local requests. Keep it as a local fixture/testing switch, not a shared-runtime impersonation path.
...
- FastestVPN can back the bounded 1min refresh lane through [docker-compose.fastestvpn.yml](docker-compose.fastestvpn.yml): place FastestVPN OpenVPN profiles under `vpn/fastestvpn/`, or fetch the reviewed Switzerland profiles with `scripts/bootstrap_fastestvpn_configs.sh`, then deploy with `EA_ENABLE_FASTESTVPN=1 bash scripts/deploy.sh`. If you use `scripts/deploy.sh`, keep that overlay explicit with `EA_ENABLE_FASTESTVPN=1`. The overlay starts only `ea-fastestvpn-proxy-ch`, binds its host port to `127.0.0.1:9315`, and gives only `ea-api` the in-network proxy address. It does not proxy worker, scheduler, WhatsApp, browser UI, or public ingress, and API startup has no VPN health dependency. The `ea-docker-socket-proxy` boundary constrains that sidecar with dropped capabilities, `no-new-privileges`, a read-only rootfs, and bounded memory/PID limits, runs the operator image under its default non-root user, and drops all ambient Linux capabilities from the API. Check the deployed boundary with `.venv/bin/python scripts/ea_live_ops.py probe-fastestvpn-transport --format operator`. Resume a private live-ops receipt with `EA_ONEMIN_DIRECT_API_PROXY_SERVER=http://127.0.0.1:9315 EA_ONEMIN_DIRECT_API_PROXY_POOL=http://127.0.0.1:9315 .venv/bin/python scripts/ea_live_ops.py refresh-onemin-direct-api --proxy-mode configured --output-json <private-receipt>`. The receipt records only proxy family, configured/reachable counts, country result, and a no-secret assertion. Use `scripts/rotate_fastestvpn_proxy.sh` to recreate the CH sidecar with `--no-build --force-recreate --no-deps` before a broad refresh.
