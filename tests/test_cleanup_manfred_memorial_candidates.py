...
def _legacy_posture() -> dict[str, object]:
    return {
        "project": LEGACY_PROJECT,
        "receipt_path": "/unused/legacy-v3.json",
        "receipt_sha256": "9" * 64,
        "observed_at": "2026-07-14T10:00:00Z",
        "image_id": IMAGE_B,
        "runtime_schema": cleanup.RUNTIME_SCHEMA_V3,
        "legacy": True,
...
class CleanupManfredMemorialCandidatesTests(unittest.TestCase):
    ...
    def test_v3_and_v4_coexist_with_legacy_quarantined(self) -> None:
        v4_rows = _project_containers(
            project=PROJECT_A,
            image_id=IMAGE_A,
        ...
        snapshot = [*v4_rows, *legacy_rows, live_row]
        ...
        self.assertEqual(report["observed_keeper"]["project"], PROJECT_A)
        self.assertEqual(
    ...
    def test_every_candidate_service_requires_exact_healthy_status(self) -> None:
        for index, health in enumerate((None, {"Status": "starting"})):
            with self.subTest(health=health):
                ...
                self.assertIsNone(report["observed_keeper"])
                docker.assert_read_only(self)
    ...
    def test_apply_remains_blocked_even_with_two_stable_v4_candidates(self) -> None:
        ...
        self.assertEqual(report["status"], "blocked")
        self.assertEqual(report["observed_keeper"]["project"], PROJECT_B)
        self.assertEqual(
        ...
        by_project = {row["project"]: row for row in report["candidates"]}
        self.assertFalse(by_project[PROJECT_B]["quarantined"])
        self.assertTrue(by_project[PROJECT_A]["quarantined"])
        self.assertEqual(
            by_project[PROJECT_A]["quarantine_reason"],
            "stable_candidate_not_observed_keeper",
        )
        self.assertEqual(
            [row["project"] for row in report["quarantined_projects"]],
            [PROJECT_A],
        )
        self.assertFalse(report["mutation_performed"])
        docker.assert_read_only(self)
