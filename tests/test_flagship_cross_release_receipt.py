...
def test_current_valid_evidence_is_deterministic_honest_and_secret_free(tmp_path: Path) -> None:
    ...
    assert first == second
    assert materialize.CONTRACT_NAME == "ea.flagship.cross_release_launch_readiness.v3"
    assert materialize.OUTPUT_NAME == "flagship-cross-release-v3.json"
...
def test_atomic_v3_write_is_mode_0600_and_never_touches_legacy_files(
    tmp_path: Path,
) -> None:
    fixture = _fixture(tmp_path)
    ...
    v2 = output_dir / "flagship-cross-release-v2.json"
    v2_bytes = b'{"schema":"ea.flagship.cross_release_launch_readiness.v2"}\n'
    v2.write_bytes(v2_bytes)
    v3 = output_dir / materialize.OUTPUT_NAME

    materialize.write_receipt(v3, receipt)
    first_bytes = v3.read_bytes()
    materialize.write_receipt(v3, receipt)

    assert v3.read_bytes() == first_bytes
    assert stat.S_IMODE(v3.stat().st_mode) == 0o600
    assert v1.read_bytes() == v1_bytes
    assert v2.read_bytes() == v2_bytes
    with pytest.raises(materialize.EvidenceValidationError) as exc:
        materialize.write_receipt(v2, receipt)
    assert "output_name_must_be_v3" in exc.value.codes


@pytest.mark.parametrize(
    "legacy_schema",
    (
        "ea.flagship.cross_release_launch_readiness.v1",
        "ea.flagship.cross_release_launch_readiness.v2",
    ),
)
def test_refuses_to_replace_legacy_schema_even_under_v3_filename(
    tmp_path: Path,
    legacy_schema: str,
) -> None:
    fixture = _fixture(tmp_path)
    ...
    _write_json(output, {"schema": legacy_schema})

    with pytest.raises(materialize.EvidenceValidationError) as exc:
        materialize.write_receipt(output, receipt)

    assert "output_refuses_non_v3_overwrite" in exc.value.codes
