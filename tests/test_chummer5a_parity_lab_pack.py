...
def test_successor_handoff_closeout_prevents_repeating_ea_scope() -> None:
    ...
    assert latest_repeat.get("result") == "registry=complete design_queue=assigned fleet_queue=complete proof=ran=15 failed=0 local_proof_commit=a57fc43"
    ...
    assert "resolving local handoff guard commit a57fc43" in str(latest_repeat.get("worker_rule") or "")
