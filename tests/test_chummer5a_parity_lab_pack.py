...
def test_successor_handoff_closeout_prevents_repeating_ea_scope() -> None:
    ...
    assert latest_repeat.get("result") == "registry=complete design_queue=assigned fleet_queue=complete proof=ran=15 failed=0 local_proof_commit=48ae7bc"
    ...
    assert "resolving local repeat proof commit 48ae7bc" in str(latest_repeat.get("worker_rule") or "")
