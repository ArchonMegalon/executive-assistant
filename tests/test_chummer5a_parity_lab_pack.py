...
def test_published_parity_oracle_receipt_matches_task_proven_pack() -> None:
    ...
    assert {"f3a3649", "528c278", "98313c9", "5d56f66", "4e6b1d8", "357ee65"} <= set(receipt_proof_commits)
...
def test_successor_handoff_closeout_prevents_repeating_ea_scope() -> None:
    ...
    assert latest_repeat.get("result") == "registry=complete design_queue=assigned fleet_queue=complete proof=ran=15 failed=0 local_proof_commit=357ee65"
    ...
    assert "resolving local handoff guard commit 357ee65" in str(latest_repeat.get("worker_rule") or "")
