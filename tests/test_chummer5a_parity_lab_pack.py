...
def test_published_parity_oracle_receipt_matches_task_proven_pack() -> None:
    ...
    assert successor_closure.get("active_handoff_min_generated_at") == "2026-04-15T14:16:41Z"
    receipt_proof_commits = [str(commit) for commit in (successor_closure.get("local_proof_commits") or [])]
    assert {"f3a3649", "528c278", "98313c9", "5d56f66"} <= set(receipt_proof_commits)
...
def test_successor_handoff_closeout_prevents_repeating_ea_scope() -> None:
    ...
    assert latest_repeat.get("result") == "registry=complete design_queue=assigned fleet_queue=complete proof=ran=15 failed=0 local_proof_commit=5d56f66"
    ...
    assert "resolving local handoff proof commit 5d56f66" in str(latest_repeat.get("worker_rule") or "")
