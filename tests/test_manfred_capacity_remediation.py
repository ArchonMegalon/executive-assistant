...
@pytest.mark.parametrize(
    ("runtime_schema", "accepted"),
    [
        (controller.RUNTIME_SCHEMA_V5, True),
        ("ea.manfred_memorial_candidate_runtime.v4", False),
    ],
)
def test_candidate_evidence_requires_current_v5_runtime(
    ...
    identity = {
        ...
        "revision": controller.EXPECTED_CANDIDATE_REVISION,
        "image_id": controller.EXPECTED_CANDIDATE_IMAGE_ID,
        "image": controller.EXPECTED_CANDIDATE_IMAGE,
    }
    monkeypatch.setattr(
        controller,
        "_read_registry_json",
        lambda _path: ({"schema": controller.REGISTRY_SCHEMA}, "8" * 64),
    )
    monkeypatch.setattr(
        controller,
        "_validated_registry",
        lambda _registry: ([entry], []),
    )
    monkeypatch.setattr(
        controller,
        "_receipt_entry",
        lambda _path: (runtime, entry, identity),
    )
    monkeypatch.setattr(
        controller,
        "_require_project_absent",
        lambda project, *, home: {"project": project, "resources_absent": True},
    )
    monkeypatch.setattr(controller, "_docker_filtered_lines", lambda *_a, **_kw: [])

    docker_calls: list[tuple[str, ...]] = []

    def bounded_run(argv: list[str], **_kwargs: object) -> bytes:
        docker_calls.append(tuple(argv))
        if argv[1:3] == ["image", "inspect"]:
            return json.dumps(
                [
                    {
                        "Id": controller.EXPECTED_CANDIDATE_IMAGE_ID,
                        "RepoTags": [controller.EXPECTED_CANDIDATE_IMAGE],
                        "RepoDigests": [],
                        "Config": {
                            "Labels": {
                                "org.opencontainers.image.revision": (
                                    controller.EXPECTED_CANDIDATE_REVISION
                                )
                            }
                        },
                    }
    ...
    monkeypatch.setattr(controller, "_bounded_run", bounded_run)
    if not accepted:
        with pytest.raises(
            RuntimeError, match="manfred_capacity_candidate_receipt_invalid"
        ):
            controller._candidate_evidence(
                home=tmp_path,
    ...
    evidence = controller._candidate_evidence(
        home=tmp_path,
        registry_path=tmp_path / "registry.json",
        protected_image_ids=set(),
    )
    assert evidence["receipt_sha256"] == "9" * 64
    assert evidence["image_unique_floor_bytes"] == 100
...
def test_root_installer_argv_uses_fixed_interpreter_and_no_shell() -> None:
    ...
    argv = controller._root_installer_argv(
        ...
        root_receipt_path=Path("/var/lib/ea/manfred-root-receipts/manfred-capacity-test.v3.json"),
    )
...
def _mock_user_apply(
    ...
    *,
    ...
    root_attestation_path = home / "root-attestation.v3.json"
...
def test_finalize_no_root_chain(
    ...
    attestation_path = tmp_path / "root-attestation.v3.json"
    attestation_path.write_text("{}\n", encoding="utf-8")
...
def test_vscode_quarantine_journal_replays_crash_boundaries(
    ...
    if crash_point == "after_rename":
        ...
        def crash_complete(path: Path, payload: dict[str, object], *, owner: int) -> str:
            nonlocal tripped
            if not tripped and path.name.endswith(".complete.v3.json"):
                tripped = True
    ...
    journal_files = list((target.parent.parent).glob("*.journal.v3.json"))
    complete_files = list((target.parent.parent).glob("*.complete.v3.json"))
...
def test_vscode_journal_contract_change_fails_before_quarantine(
    ...
    assert list((target.parent.parent).glob("*.journal.v3.json")) == []
...
def test_projection_deletion_journal_replays_each_crash_boundary(
    ...
    journal_files = list(Path(state["quarantine"]).glob("*.journal.v3.json"))
    complete_files = list(Path(state["quarantine"]).glob("*.complete.v3.json"))
    assert len(journal_files) == len(complete_files) == 1
...
def test_schema_v3_cut_rejects_v2_plan() -> None:
    schemas = (
    ...
    assert all(value.endswith(".v3") for value in schemas)
    plan = _plan_scope()
    plan["schema"] = "ea.manfred_memorial_build_capacity.plan.v2"
...
def test_generic_root_candidate_journal_is_durable_before_rename(
    ...
    def crash_before_rename(*_args: object, **_kwargs: object) -> None:
        assert len(list(quarantine.glob("*.journal.v3.json"))) == 1
    ...
    assert target.exists()
    assert len(list(quarantine.glob("*.journal.v3.json"))) == 1
    assert list(quarantine.glob("*.complete.v3.json")) == []
...
def test_generic_root_candidate_replays_every_journaled_crash_boundary(
    ...
    assert len(list(quarantine.glob("*.journal.v3.json"))) == 1
    assert len(list(quarantine.glob("*.complete.v3.json"))) == 1
