...
def test_prepared_projection_never_grants_authority() -> None:
    result = _verify()
    assert result["contract_name"] == "ea.audiobook_runtime_production_preflight.v2"
    ...
    assert (
        ...
        == "ea.audiobook_runtime_production_projection.v2"
    )
...
def test_root_authority_anchor_requires_uid0_directory_and_safe_mode() -> None:
    ...
    assert production._root_anchor_is_trusted(trusted, trusted) is True
    adversarial = (
    ...
    for candidate in adversarial:
        assert production._root_anchor_is_trusted(trusted, candidate) is False


def test_root_authority_traversal_checks_initial_root_anchor(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(
        production,
        "_root_anchor_is_trusted",
        lambda _opened, _current: False,
    )
    with pytest.raises(
        production.ProductionStageError,
        match="trusted_input_root_anchor_untrusted",
    ):
        production._open_absolute_nofollow(
            Path("/etc/hosts"),
            flags=os.O_RDONLY,
            reason="trusted_input",
...
def test_missing_promisor_blob_never_fetches_or_invokes_helper(
    ...
    upload_pack = tmp_path / "upload-pack-marker"
    setup_environment = {
...
def test_trusted_reader_rejects_duplicate_authority_keys(tmp_path: Path) -> None:
    ...
    with pytest.raises(production.ProductionStageError, match="authority_json_invalid"):
        production.read_trusted_json(
            authority,
...
def test_producer_has_no_authorize_mode() -> None:
