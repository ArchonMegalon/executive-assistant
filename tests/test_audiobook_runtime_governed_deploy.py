...
def _production_authority(
    ...
    *,
    ...
    projection = {
        "contract_name": deploy.PRODUCTION_PROJECTION_CONTRACT,
        "version": deploy.PRODUCTION_PROJECTION_VERSION,
        "status": "prepared",
        "configuration_only": True,
    ...
    return {
        ...
        "version": deploy.PRODUCTION_PREFLIGHT_VERSION,
        "status": "prepared",
        ...
        **denied,
        ...
        "next_action": "governed_consumer_may_create_paused_stage_after_live_preflight",
    }
...
def test_consumer_constants_match_exact_producer_contract() -> None:
    assert deploy.DEPLOY_RECEIPT_CONTRACT == "ea.audiobook_runtime_governed_deploy.v2"
    assert deploy.PRODUCTION_PREFLIGHT_CONTRACT == producer.ROOT_CONTRACT
    assert deploy.PRODUCTION_PREFLIGHT_VERSION == producer.ROOT_CONTRACT_VERSION == 2
    assert deploy.PRODUCTION_PROJECTION_CONTRACT == producer.PROJECTION_CONTRACT
    assert (
        deploy.PRODUCTION_PROJECTION_VERSION
        == producer.PROJECTION_CONTRACT_VERSION
        == 2
    )
...
def test_governed_deploy_receipt_uses_policy_free_v2_contract(tmp_path: Path) -> None:
    lane = _lane(tmp_path)

    assert lane.receipt["contract_name"] == "ea.audiobook_runtime_governed_deploy.v2"


def test_production_root_owner_is_fixed() -> None:
    lane = object.__new__(deploy.AudiobookRuntimeDeployLane)
    assert lane.root_authority_uid == 0


def test_evidence_owner_must_be_explicit(tmp_path: Path) -> None:
    root = _root(tmp_path)
    env = {
        "EA_DEPLOYMENT_ID": "audiobook-test-001",
    }
    with pytest.raises(deploy.DeployError, match="explicit_evidence_owner_uid_required"):
...
@pytest.mark.parametrize(
    ...
    (
        ("preflight", "approved_production_configuration_contract_required"),
        ("projection", "production_projection_invalid"),
    ),
)
def test_policy_bearing_v1_production_contracts_are_rejected(
    tmp_path: Path,
    target: str,
    expected_reason: str,
) -> None:
    ...
    if target == "preflight":
        payload["contract_name"] = "ea.audiobook_runtime_production_preflight.v1"
        payload["version"] = 1
    else:
        payload["production_projection"][
            "contract_name"
        ] = "ea.audiobook_runtime_production_projection.v1"
        payload["production_projection"]["version"] = 1
    _write_json(lane.configuration_receipt_path, payload)

    with pytest.raises(deploy.DeployError, match=expected_reason):
        lane._read_evidence()
...
def test_live_evidence_guard_revalidates_before_transaction(tmp_path: Path) -> None:
    lane = _lane(tmp_path)
    boundaries: list[str] = []
    lane._revalidate_evidence = types.MethodType(
        lambda self, boundary: boundaries.append(boundary), lane
    )
    with lane._live_evidence_guard():
        boundaries.append("mutation")
    assert boundaries == [
        "immediately_before:paused_stage_transaction",
        "mutation",
    ]
...
def test_execute_path_without_preconditions_runs_no_command(tmp_path: Path) -> None:
    lane = _lane(tmp_path)
...
def _authorized_transaction_lane(
    ...
    events: list[str] = []
    lane._revalidate_evidence = types.MethodType(
        lambda self, boundary: events.append(f"evidence:{boundary}"), lane
    )
...
def test_real_preflight_prepares_bound_plan_before_fake_execute(
    tmp_path: Path,
) -> None:
    rendered = {
        "services": {
...
def test_authorized_fake_creates_exact_paused_stage_without_activation(
    ...
    assert events[:13] == [
        "evidence:immediately_before:paused_stage_transaction",
        "candidate",
        "target_config",
        ...
        "evidence:before:paused_stage_create",
        "candidate",
        "target_config",
        "prestate:" + ",".join(deploy.EXPECTED_RUNTIME_SERVICES),
        "controls",
