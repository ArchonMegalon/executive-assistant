...
_COLLECTION_ENVIRONMENT_BASELINE = dict(os.environ)
...
def pytest_collection_finish(session: pytest.Session) -> None:
    # Some CLI-oriented modules load the repository's local runtime environment
    # when pytest imports them for collection.  Tests must start from the
    # process environment that launched pytest, never from local deployment
    # secrets or production routing settings discovered during collection.
    del session
    _restore_environment(_COLLECTION_ENVIRONMENT_BASELINE)
