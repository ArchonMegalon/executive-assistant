...
def test_plan_is_exactly_non_authoritative_and_unverified(tmp_path: Path) -> None:
    ...
    assert payload["contract_name"] == planner.PLAN_CONTRACT
    ...
    assert payload["authority"]["required_permit_checkpoints"] == [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("authority", "joint_permit_contract", "ea.unreviewed.v1"),
        ("authority", "required_permit_checkpoints", ["before_recreate_api"]),
        ("execution", "blocker", "ready_to_execute"),
        ("execution", "docker_mutations", False),
        ("recovery_requirements", "permit_free_rollback_required", False),
        (
            "identity_requirements",
            "permitted_differences",
            [
...
def test_plan_contract_cannot_be_promotion_or_candidate_evidence(
    ...
    assert payload["contract_name"] not in {
        "ea.memorial_joint_api_ingress_deploy.v1",
        "ea.memorial_scoped_deploy_receipt.v1",
