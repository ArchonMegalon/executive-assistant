...
def test_capacity_projection_omits_empty_magicx_without_leaking_secrets() -> None:
    ...
    assert rows["1min.AI"]["credit_balance_state"] == "unverified"
    assert rows["1min.AI"]["slot_health_state"] == "unverified"
    assert rows["1min.AI"]["dispatch_eligible"] is False
    assert rows["AI Magicx"]["route_state"] == "omitted_empty_credential"
    assert payload["status"] == "ready_for_bounded_probe"
    assert payload["eligibility_scope"] == "health_probe_only"
    assert payload["probe_candidate_count"] == 1
    assert payload["dispatch_eligible_provider_count"] == 0
...
def test_proof_debt_has_one_complete_row_for_each_non_tier_one_inventory_entry() -> None:
    ...
    lanes_by_service = {row["service"]: row["candidate_lane"] for row in rows}
    assert lanes_by_service["ClickRank.ai"] == "release_trust_factory"
    assert lanes_by_service["FastestVPN PRO"] == "onemin_refresh_only"
    assert lanes_by_service["VocalLab.ai"] == "voice_catalog_only"
