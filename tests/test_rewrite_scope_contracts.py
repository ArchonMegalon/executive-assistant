...
def test_principal_scoped_rewrite_fetch_helpers_enforce_ownership() -> None:
    ...
    artifact = orchestrator.build_artifact(RewriteRequest(text="scope-check", principal_id="principal-default"))
    ...
    scoped_session = orchestrator.fetch_session_for_principal(session.session.session_id, principal_id="principal-default")
    assert scoped_session is not None
    assert scoped_session.session.intent.principal_id == "principal-default"

    scoped_artifact = orchestrator.fetch_artifact_for_principal(artifact.artifact_id, principal_id="principal-default")
    assert scoped_artifact is not None
    assert scoped_artifact[0].artifact_id == artifact.artifact_id
    assert scoped_artifact[0].principal_id == "principal-default"
    assert scoped_artifact[1].session.session_id == session.session.session_id

    receipt_id = session.receipts[0].receipt_id
    scoped_receipt = orchestrator.fetch_receipt_for_principal(receipt_id, principal_id="principal-default")
    ...
    scoped_cost = orchestrator.fetch_run_cost_for_principal(cost_id, principal_id="principal-default")
