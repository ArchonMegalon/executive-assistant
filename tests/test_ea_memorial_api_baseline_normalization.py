...
def test_forward_uses_exact_two_permits_after_durable_phase_boundaries(
    monkeypatch: pytest.MonkeyPatch,
    lane_factory: Callable[..., normalization.ApiBaselineNormalizationLane],
) -> None:
    events: list[str] = []
    ...
    rollback_tag = "ea-runtime:normalization-rollback-operation-0001"
    prepared = {
        "live": {"expected_image_id": IMAGE_ID},
    ...
    @contextmanager
    def permit(boundary: str) -> Iterator[None]:
        permits.append(boundary)
        events.append(f"permit:enter:{boundary}")
        try:
            yield
        finally:
            events.append(f"permit:exit:{boundary}")

    def revalidate(
        _prepared: Mapping[str, Any], *, protected_image_required: bool
    ) -> dict[str, Any]:
        events.append(f"revalidate:{protected_image_required}")
        return {}
    ...
    def run(
        ...
        del check, cwd
        assert list(args) == ["docker", "image", "tag", IMAGE_ID, rollback_tag]
        events.append("mutation:image-tag")
        return subprocess.CompletedProcess(list(args), 0, "", "")

    def recreate(_bundle_value: Mapping[str, Any]) -> None:
        events.append("mutation:compose-up")

    def capture_terminal(
        *, prepared: Mapping[str, Any], payload: Mapping[str, Any]
    ) -> dict[str, Any]:
        del prepared, payload
        events.append("capture:terminal")
        return {"runtime": {"api_identity": {"schema": "test"}}}

    def commit(
        _payload: Mapping[str, Any], _terminal: Mapping[str, Any]
    ) -> dict[str, Any]:
        events.append("commit")
        return {"status": "pass"}

    monkeypatch.setattr(lane, "_vexp_mutation_lease", permit)
    monkeypatch.setattr(lane, "_remaining_vexp_mutation_seconds", lambda: 60.0)
    monkeypatch.setattr(lane, "_revalidate_forward_baseline", revalidate)
    monkeypatch.setattr(lane, "_inspect_image_optional", lambda _ref: None)
    monkeypatch.setattr(lane, "_run_sanitized", run)
    monkeypatch.setattr(
        lane,
        "_require_protected_image",
        lambda _tag, _image_id: {"Id": IMAGE_ID, "RepoTags": [rollback_tag]},
    )
    monkeypatch.setattr(lane, "_sealed_api_recreate", recreate)
    monkeypatch.setattr(lane, "_capture_terminal_forward_evidence", capture_terminal)
    monkeypatch.setattr(lane, "_new_forward_journal_payload", lambda _value: initial)
    monkeypatch.setattr(lane, "_durable_forward_commit", commit)
    monkeypatch.setattr(
        lane,
        "_recover_forward_failure",
        lambda *_args, **_kwargs: pytest.fail("happy forward path entered recovery"),
    )

    assert lane._normalize(prepared) == {"status": "pass"}
    assert permits == [
        "before_protect_previous_image",
        "before_recreate_api",
    ]
    assert events.index("journal:create") < events.index(
        "permit:enter:before_protect_previous_image"
    )
    assert events.index("journal:phase:protect_previous_image_possible") < events.index(
        "mutation:image-tag"
    )
    assert events.index("journal:phase:api_mutation_possible") < events.index(
        "mutation:compose-up"
    )
    assert [event for event in events if event.startswith("permit:enter:")] == [
        "permit:enter:before_protect_previous_image",
        "permit:enter:before_recreate_api",
    ]
...
def test_recovery_uses_only_journal_authority_and_never_requests_a_permit(
    monkeypatch: pytest.MonkeyPatch,
    lane_factory: Callable[..., normalization.ApiBaselineNormalizationLane],
) -> None:
    payload = {
    ...
    monkeypatch.setattr(lane, "_vexp_mutation_lease", forbidden)
    ...
    assert lane.receipt["recovery_permit_requested"] is False
...
def test_old_baseline_recovery_persists_phase_before_exact_tag_removal(
    ...
    monkeypatch.setattr(
        lane,
        "_vexp_mutation_lease",
        lambda *_args, **_kwargs: pytest.fail("rollback recovery requested a permit"),
    )
...
def test_forward_recovery_reseals_after_terminal_capture_before_attachment(
    ...
    monkeypatch.setattr(
        lane,
        "_vexp_mutation_lease",
        lambda *_args, **_kwargs: pytest.fail("recovery requested a permit"),
    )
