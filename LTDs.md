...
|---|---|---|---|---|---|---|---|
| `1min.AI` | `Advanced Business Plan` | `12 licenses / 12 accounts` | `Owned` |  | `Tier 1` | Local `.env` key rotation slots plus `scripts/resolve_onemin_ai_key.sh` | Primary and fallback API-key flow is wired locally and kept out of git. Shared browser-login password is seeded in local `.env`. Latest credit refresh on `2026-05-31T09:06:54.154808+00:00` for `ONEMIN_AI_API_KEY` confirmed `60279` remaining credits without a projected next top-up in the latest refresh. |
...
|---|---|---|---|---|---|
| `1min.AI` |  | `manual_seeded` | `local_env` | 2026-05-31T09:06:54.154808+00:00 | API-key rotation slots and the shared browser-login password now exist locally. Latest credit refresh on `2026-05-31T09:06:54.154808+00:00` for `ONEMIN_AI_API_KEY` confirmed `60279` remaining credits without a projected next top-up in the latest refresh. |
