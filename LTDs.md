...
|---|---|---|---|---|---|---|---|
| `1min.AI` | `Advanced Business Plan` | `12 licenses / 12 accounts` | `Owned` |  | `Tier 1` | Local `.env` key rotation slots plus `scripts/resolve_onemin_ai_key.sh` | Primary and fallback API-key flow is wired locally and kept out of git. Shared browser-login password is seeded in local `.env`. Latest credit refresh on `2026-06-05T15:53:33.520680+00:00` for `ONEMIN_AI_API_KEY` confirmed `12345` remaining credits with the next top-up projected for `2026-03-31T00:00:00Z` (`20000` credits). |
...
| `FlipLink.me` | `Tier 10` | `1 account / high-capacity flipbook plan` | `Owned` |  | `Tier 3` | None yet; candidate Chummer Document Portal after provider verification | Acquired for branded public/private PDF-to-flipbook publishing. Candidate uses: Chummer6 public guides, install booklets, Black Ledger primers, faction dossiers, Table Pulse guides, GM Video Foundry guides, convention/launch handouts, and player-safe campaign recap packets. FlipLink may present, embed, and measure approved Chummer documents only; it must not host sourcebook PDFs, copied rulebook prose, private runner sheets, GM-only campaign secrets, or entitlement/payment truth. Promote to Tier 2 only after provider verification, CNAME/API/embed/analytics/password/access proof, copyright/privacy boundary proof, and first Chummer flipbook publication receipt. |
...
| `FlipLink.me` |  | `manual_seeded` | `user_reported` | 2026-06-05T00:00:00Z | Tier 10 reported by the user. Candidate Document Portal for Chummer6 guides, Black Ledger player-safe dossiers, GM packets, and release/convention flipbooks. Provider verification, dashboard capacity, CNAME/API/embed/analytics/password protection, access controls, and copyright/privacy publishing proof are pending. |
