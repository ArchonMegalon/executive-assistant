...
|---|---|---|---|---|---|---|---|
| `1min.AI` | `Advanced Business Plan` | `12 licenses / 12 accounts` | `Owned` |  | `Tier 1` | Local `.env` key rotation slots plus `scripts/resolve_onemin_ai_key.sh` | Primary and fallback API-key flow is wired locally and kept out of git. Shared browser-login password is seeded in local `.env`. Latest credit refresh on `2026-06-01T17:17:37.916772+00:00` for `ONEMIN_AI_API_KEY` confirmed `12345` remaining credits with the next top-up projected for `2026-03-31T00:00:00Z` (`20000` credits). |
...
| `Prompt Architects` | `Tier 4` | `1 account` | `Activated` |  | `Tier 4` | `PROMPTING_SYSTEMS_API_KEY` in local `.env`; `Prompting Systems` adapter remains a bounded style/prompt helper | AppSumo Tier 4 capture is confirmed; account-level verification remains manual and runtime lane is intentionally kept limited to bounded guide workflows. |
...
|---|---|---|---|---|---|
| `1min.AI` |  | `manual_seeded` | `local_env_browseract_refresh` | 2026-06-01T17:17:37.916772+00:00 | API-key rotation slots and the shared browser-login password now exist locally. Latest credit refresh on `2026-06-01T17:17:37.916772+00:00` for `ONEMIN_AI_API_KEY` confirmed `12345` remaining credits with the next top-up projected for `2026-03-31T00:00:00Z` (`20000` credits). |
| `Prompt Architects` |  | `manual_seeded` | `local_env` | 2026-06-01T00:00:00Z | Local `.env` now contains the AppSumo API key slot for `PROMPTING_SYSTEMS_API_KEY`; structured account verification remains pending. |
