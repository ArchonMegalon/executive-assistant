...
|---|---|---|---|---|---|---|---|
| `1min.AI` | `Advanced Business Plan` | `12 licenses / 12 accounts` | `Owned` |  | `Tier 1` | Local `.env` key rotation slots plus `scripts/resolve_onemin_ai_key.sh` | Primary and fallback API-key flow is wired locally and kept out of git. Shared browser-login password is seeded in local `.env`. Full BrowserAct refresh plus retries finished on `2026-05-31T10:20:54Z` and observed `2,726,573` total remaining credits across the current account pool. Primary key `ONEMIN_AI_API_KEY` still reports `60279` remaining credits without a projected next top-up. |
...
|---|---|---|---|---|---|
| `1min.AI` |  | `manual_seeded` | `local_env_browseract_refresh` | 2026-05-31T10:20:54Z | API-key rotation slots and the shared browser-login password now exist locally. Full BrowserAct refresh plus retries observed `2,726,573` total remaining credits across the current account pool; primary `ONEMIN_AI_API_KEY` confirmed `60279` remaining credits without a projected next top-up. |
