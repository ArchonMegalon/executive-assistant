...
- The current `chummer6_guide_worker.py` generation path auto-registers missing Chummer6 skill contracts locally, seeds itself from public guide canon, runs `chummer6_public_writer` and a targeted rewrite pass for weak copy, then `chummer6_visual_director` -> the four explicit auditor skills (`chummer6_public_auditor`, `chummer6_scene_auditor`, `chummer6_visual_auditor`, `chummer6_pack_auditor`) -> local scene/editorial pack audits.
- The Chummer6 public-guide lane is registry-first now: the active contract reads `public_page_registry`, `public_part_registry`, `public_faq_registry`, `public_status`, and the generated public export surface. Older broad-input framing like `repo_readmes` or a single `design_scope` summary is not the current guide-generation truth.
