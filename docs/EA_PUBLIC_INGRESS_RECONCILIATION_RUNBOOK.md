...
The API-only permit authorizes only the exact API/Redis boundaries consumed by
`scripts/deploy_ea_memorial.py`; it has no cloudflared boundary.
`scripts/reconcile_ea_public_ingress.py` therefore rejects every standalone
mutation request with `public_ingress_reconciliation_coordinator_required`. Do
not relabel an API boundary, bypass the guard, or run the generated Compose
topology manually.

The only joint mutation lane is `scripts/deploy_ea_memorial_joint.py`. It
requires the distinct root-issued joint permit and its exact fourth
`before_recreate_cloudflared` boundary, owns API/ingress/network/public-edge
baselines in one rollback domain, and emits an explicit joint receipt. Use
`make verify-ea-memorial-joint-deploy` for its non-mutating preflight and
`make deploy-ea-memorial-joint` only inside the authorized terminal window.
The standalone reconciliation lane remains read-only and makes no container,
network, Docker, systemd, sentinel, qualification, certificate, AppArmor,
event-guard, or mutation-gate change.
