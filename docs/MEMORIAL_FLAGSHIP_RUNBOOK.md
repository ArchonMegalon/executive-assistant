...
A passing source gate is the first release check. Continue with the isolated
candidate and joint preflight described below; neither source-only nor
candidate-only evidence substitutes for the public-origin proof.
...
Its scene is event-driven rather than continuously rendered, allows native
...
## Manfred production deployment

Flagship source evidence is not deployment evidence. After source and 3D inputs
are ready, run the source gate, build and verify the isolated candidate and
exact-tour proof, run the non-mutating joint preflight, deploy through the joint
API-and-ingress coordinator, then complete credential-free public browser and
3D proof. See `MANFRED_MEMORIAL_SCOPED_DEPLOY_RUNBOOK.md` for the candidate and
API-component contract and `MANFRED_MEMORIAL_JOINT_DEPLOY_RUNBOOK.md` for the
joint rollback and public-evidence sequence. Do not bypass either lane with raw
