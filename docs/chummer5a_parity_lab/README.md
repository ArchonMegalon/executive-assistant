...
- The active handoff `Mode:` field is assignment metadata only. `Mode: unknown` does not reopen this EA package when frontier/package identity, canonical registry and queue rows, completed outputs, and the direct proof command still agree.
