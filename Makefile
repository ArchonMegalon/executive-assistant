...
test-api:
	...
	CI=$${CI:-1} EA_RUNTIME_MODE=test PYTHONPATH=ea EA_STORAGE_BACKEND=memory $(PYTHON_BIN) -m pytest -q tests $(TEST_API_PYTEST_IGNORE) $(TEST_API_PYTEST_DESELECT)
...
ltd-release-gates:
	...
	$(MAKE) verify-ltd-operating-mesh

materialize-ltd-proof-debt:
	$(PYTHON_BIN) scripts/materialize_ltd_proof_debt.py

materialize-ltd-capacity-status:
	$(PYTHON_BIN) scripts/materialize_ltd_capacity_status.py

verify-ltd-operating-mesh:
	$(PYTHON_BIN) scripts/verify_ltd_capacity_scheduler.py
	@mesh_dir=$$(mktemp -d); \
		trap 'rm -rf "$$mesh_dir"' EXIT; \
		$(PYTHON_BIN) scripts/materialize_ltd_capacity_status.py --output "$$mesh_dir/provider.json"; \
		$(PYTHON_BIN) scripts/materialize_ltd_proof_debt.py --output "$$mesh_dir/proof.json"; \
		$(PYTHON_BIN) scripts/sync_ltd_operations_to_teable.py \
			--provider-status-path "$$mesh_dir/provider.json" \
			--proof-debt-path "$$mesh_dir/proof.json"
	PYTHONPATH=ea $(PYTHON_BIN) -m pytest -q tests/test_ltd_operating_mesh.py tests/test_ltd_teable_operations.py

sync-ltd-operations-teable:
	$(PYTHON_BIN) scripts/sync_ltd_operations_to_teable.py --apply --create-missing

probe-fastestvpn-transport:
	$(PYTHON_BIN) scripts/ea_live_ops.py probe-fastestvpn-transport --format operator

query-ltd-opportunities:
	$(PYTHON_BIN) scripts/query_ltd_opportunity_index.py --execute
...
all-local: ci-local verify-codexea-e2e-exit-gate verify-codexea-fleet-shim-parity verify-local-quality-gates verify-release-assets verify-flagship-release-readiness verify-generated-release-artifacts-clean
