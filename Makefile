...
.PHONY: deploy-ea-memorial-joint recover-ea-memorial-joint verify-ea-memorial-joint-deploy
...
recover-ea-memorial-joint:
	@ : "$${EA_DEPLOYMENT_ID:?Set a fresh explicit EA_DEPLOYMENT_ID before joint memorial recovery}"
	COMPOSE_PROJECT_NAME=ea \
	EA_DEPLOY_PRIMARY_MODE=MEMORIAL \
	EA_DEPLOY_ENABLED_MODES=MEMORIAL \
	EA_DEPLOY_COMPOSE_OVERRIDES=docker-compose.memorial.yml \
	$(PYTHON_BIN) scripts/deploy_ea_memorial_joint.py --recover-active
