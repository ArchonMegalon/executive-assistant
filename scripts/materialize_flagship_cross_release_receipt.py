...
CONTRACT_NAME = "ea.flagship.cross_release_launch_readiness.v3"
OUTPUT_NAME = "flagship-cross-release-v3.json"
...
EVIDENCE_SPECS = (
    EvidenceSpec("manfred_runtime", "schema", "ea.manfred_memorial_candidate_runtime.v5"),
    ...
    payload: dict[str, object] = {
        ...
        "long_running_goal": {
            ...
            "objective": (
                ...
                "and promote only after every owner-controlled authority, deployment, and provider-tour gate is green."
            ),
...
def write_receipt(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if destination.name != OUTPUT_NAME:
        raise EvidenceValidationError("output_name_must_be_v3")
    ...
    if existing_stat is not None:
        ...
        if not isinstance(existing, dict) or existing.get("schema") != CONTRACT_NAME:
            raise EvidenceValidationError("output_refuses_non_v3_overwrite")
