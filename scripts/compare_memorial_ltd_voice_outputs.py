...
_REFERENCE_AUDIO_ENV = "EA_MEMORIAL_LTD_REFERENCE_AUDIO"
_REFERENCE_AUDIO_RELATIVE_PATH = (
    Path("manfred")
    / "voice_profile"
    / "optimization"
    / "candidates"
...
def _required_unmixr_voice_id() -> str:
    voice_id = unmixr_memorial_voice_id()
    if not voice_id:
        raise RuntimeError("unmixr_voice_id_missing")
    return voice_id


def _reference_audio_path() -> Path:
    configured = str(os.environ.get(_REFERENCE_AUDIO_ENV) or "").strip()
    if configured:
        return Path(configured).expanduser()
    return private_profile_dir() / _REFERENCE_AUDIO_RELATIVE_PATH
...
def _render_voicewave(*, text: str, audio_output: Path, json_output: Path, screenshot_output: Path) -> bytes:
    ...
    script = ROOT / "scripts" / "voicewave_memorial_voice.py"
...
def _compare_prompt(*, prompt: str, base_url: str, output_dir: Path) -> dict[str, object]:
    reference = _reference_audio_path()
    if not reference.is_file():
        raise RuntimeError("memorial_ltd_reference_audio_missing")
    ...
    try:
        unmixr_audio, unmixr_content_type = unmixr_synthesize_request(
            text=prompt,
            voice_id=_required_unmixr_voice_id(),
            lang="de-AT",
