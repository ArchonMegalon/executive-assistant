...
def _run_worker(*, login_email: str, login_password: str, page_url: str, timeout_seconds: int) -> dict[str, object]:
    ...
    packet = {
        ...
        "workflow_spec_json": browseract_ui_template_spec("voicewave_workspace_reader"),
        "browseract_username": login_email,
        "browseract_password": login_password,
    ...
    if not raw:
        ...
        raise RuntimeError(f"voicewave_analysis_worker_empty_output:{detail[:400]}")
    last_line = raw.splitlines()[-1].strip()
    try:
        payload = json.loads(last_line)
    except Exception as exc:
        raise RuntimeError(f"voicewave_analysis_worker_non_json:{last_line[:400]}") from exc
    if not isinstance(payload, dict):
        raise RuntimeError("voicewave_analysis_worker_invalid_output")
...
def _worker_summary(result: dict[str, object]) -> dict[str, object]:
    ...
    lowered_haystack = " ".join((title, url, body_text)).lower()
    homepage_markers = (
        "get lifetime access",
        "lifetime access",
        "money back",
        "for creators, marketers & businesses",
        "copyright ©",
    )
    workspace_markers = (
        "/app",
    ...
    looks_marketing_only = any(marker in lowered_haystack for marker in homepage_markers)
    looks_workspace = any(marker in lowered_haystack for marker in workspace_markers)
    authenticated = (
        render_status in {"completed", "completed_with_warnings"}
        and ui_failure_code == ""
        and bool(title or body_text or url)
        and looks_workspace
        and not looks_marketing_only
    )
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze a VoiceWave workspace surface for memorial voice-lane fit.")
    parser.add_argument("--login-email", default="")
    parser.add_argument("--login-password", default="")
    parser.add_argument("--project-name", default="Manfred Memorial Voice")
