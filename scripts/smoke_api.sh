...
if [[ "${SESSION_RUNTIME_FIELDS}" != "completed|True|True|True|True|True|True|True|True|smoke run|artifact://${ARTIFACT_ID}|${PRINCIPAL_ID}" ]]; then
  echo "expected initial rewrite session to complete with ordered queued input/policy events, done queue items, dependency-state projection metadata, empty human-task assignment history, and artifact envelope ownership fields; got ${SESSION_RUNTIME_FIELDS}" >&2
  echo "${SESSION_JSON}" >&2
  fail 12 "policy contract mismatch"
fi
