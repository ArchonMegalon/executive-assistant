...
export SMOKE_TMP_DIR="${EA_SMOKE_TMP_DIR:-${EA_ROOT}/.smoke_tmp}"
...
Auth:
  ...
  (default: principal-default) and verify mismatches against EA_MISMATCH_PRINCIPAL_ID
...
PRINCIPAL_ID="${EA_PRINCIPAL_ID:-principal-default}"
...
mkdir -p "${SMOKE_TMP_DIR}"
REGISTER_COOKIE_JAR="${SMOKE_TMP_DIR}/register_workspace_access.cookies"
REGISTER_ACCESS_HEADERS="${SMOKE_TMP_DIR}/register_workspace_access.headers"
...
REVOKED_ACCESS_STATUS="$(curl_status_code "${SMOKE_TMP_DIR}/revoked_workspace_access.body" "${BASE}${ACCESS_SESSION_URL}")"
if [[ "${REVOKED_ACCESS_STATUS}" != "404" ]]; then
  echo "expected revoked workspace access link to return 404; got ${REVOKED_ACCESS_STATUS}" >&2
  cat "${SMOKE_TMP_DIR}/revoked_workspace_access.body" >&2 || true
  fail 12 "policy contract mismatch"
fi
...
GOOGLE_SYNC_BODY="${SMOKE_TMP_DIR}/google_signal_sync.json"
...
HUMAN_CREATE_MISMATCH_REASON="$(python3 -c 'import json, os; from pathlib import Path; body=json.loads(((Path(os.environ["SMOKE_TMP_DIR"]) / "ea_human_create_mismatch_resp.json").read_text()) or "{}"); print((body.get("error") or {}).get("code",""))')"
...
HUMAN_SESSION_LIST_MISMATCH_REASON="$(python3 -c 'import json, os; from pathlib import Path; body=json.loads(((Path(os.environ["SMOKE_TMP_DIR"]) / "ea_human_session_list_mismatch_resp.json").read_text()) or "{}"); print((body.get("error") or {}).get("code",""))')"
...
path = Path(os.environ["SMOKE_TMP_DIR"]) / "ea_approval_required_resp.json"
if not path.exists():
    print("")
    raise SystemExit(0)
try:
...
import os
from pathlib import Path
path = Path(os.environ["SMOKE_TMP_DIR"]) / "ea_blocked_policy_resp.json"
if not path.exists():
    print("")
    raise SystemExit(0)
try:
...
path = Path(os.environ["SMOKE_TMP_DIR"]) / "ea_tool_exec_mismatch_resp.json"
if not path.exists():
    print("")
    raise SystemExit(0)
try:
...
path = Path(os.environ["SMOKE_TMP_DIR"]) / "ea_connector_mismatch_resp.json"
if not path.exists():
    print("")
    raise SystemExit(0)
try:
...
CHUMMER_PUBLIC_WRITER_JSON="$(operator_post_json "${BASE}/v1/skills" -H 'content-type: application/json' \
  -d '{"skill_key":"chummer6_public_writer","task_key":"chummer6_public_copy_refresh","name":"Chummer6 Public Writer","description":"Planner-executed public-writer lane for Chummer6 guide copy, audience translation, and reader-safe OODA framing.","deliverable_type":"chummer6_guide_refresh_packet","default_risk_class":"low","default_approval_class":"none","workflow_template":"tool_then_artifact","allowed_tools":["provider.brain_router.structured_generate","artifact_repository"],"evidence_requirements":["repo_readmes","design_scope","public_status","source_prompt"],"memory_write_policy":"reviewed_only","memory_reads":["entities","relationships","repo_readmes","design_scope","public_status"],"memory_writes":["chummer6_public_copy_fact"],"tags":["chummer6","guide","public-writer","audience","copy"],"authority_profile_json":{"authority_class":"draft","review_class":"operator"},"model_policy_json":{"brain_profile":"groundwork","default_model":"ea-groundwork","posthoc_review_profile":"review_light","fallback_brain_profile":"survival","output_mode":"json"},"provider_hints_json":{"primary":["1min.AI"],"research":["BrowserAct"],"output":["1min.AI","Prompting Systems","Gemini Vortex"],"style":["1min.AI","Gemini Vortex"]},"tool_policy_json":{"allowed_tools":["provider.brain_router.structured_generate","artifact_repository"]},"human_policy_json":{"review_roles":["guide_reviewer"]},"evaluation_cases_json":[{"case_key":"chummer6_guide_refresh_golden","priority":"medium"}],"budget_policy_json":{"class":"low","workflow_template":"tool_then_artifact","pre_artifact_capability_key":"structured_generate","artifact_failure_strategy":"retry","artifact_max_attempts":2,"artifact_retry_backoff_seconds":1,"style_epoch_enabled":true,"variation_guard_enabled":true}}')"
...
CHUMMER_PUBLIC_WRITER_FIELDS="$(python3 -c "import json,sys; created=json.loads(sys.argv[1]); fetched=json.loads(sys.argv[2]); compiled=json.loads(sys.argv[3]); steps=compiled.get('plan',{}).get('steps') or []; print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(created.get('skill_key',''), created.get('task_key',''), created.get('workflow_template',''), ','.join((created.get('provider_hints_json') or {}).get('primary') or []), fetched.get('task_key',''), (fetched.get('model_policy_json') or {}).get('brain_profile',''), compiled.get('skill_key',''), len(steps), ','.join(step.get('step_key','') for step in steps), steps[1].get('tool_name','') if len(steps) > 1 else ''))" "${CHUMMER_PUBLIC_WRITER_JSON}" "${CHUMMER_PUBLIC_WRITER_FETCH_JSON}" "${CHUMMER_PUBLIC_WRITER_PLAN_JSON}")"
if [[ "${CHUMMER_PUBLIC_WRITER_FIELDS}" != "chummer6_public_writer|chummer6_public_copy_refresh|tool_then_artifact|1min.AI|chummer6_public_copy_refresh|groundwork|chummer6_public_writer|3|step_input_prepare,step_structured_generate,step_artifact_save|provider.brain_router.structured_generate" ]]; then
  echo "expected chummer6_public_writer to compile through the EA groundwork structured-generation lane; got ${CHUMMER_PUBLIC_WRITER_FIELDS}" >&2
...
CHUMMER_SKILL_JSON="$(operator_post_json "${BASE}/v1/skills" -H 'content-type: application/json' \
  -d '{"skill_key":"chummer6_visual_director","task_key":"chummer6_guide_refresh","name":"Chummer6 Visual Director","description":"Planner-executed Chummer6 scene planning, style-epoch selection, scene-ledger guidance, and structured visual-direction skill for the public-facing guide.","deliverable_type":"chummer6_guide_refresh_packet","default_risk_class":"low","default_approval_class":"none","workflow_template":"tool_then_artifact","allowed_tools":["provider.brain_router.structured_generate","artifact_repository"],"evidence_requirements":["repo_readmes","design_scope","public_status","source_prompt"],"memory_write_policy":"reviewed_only","memory_reads":["entities","relationships","repo_readmes","design_scope","public_status"],"memory_writes":["chummer6_style_epoch","chummer6_scene_ledger","chummer6_visual_critic_fact"],"tags":["chummer6","guide","visual-direction","style-epoch","scene-ledger"],"authority_profile_json":{"authority_class":"draft","review_class":"operator"},"model_policy_json":{"brain_profile":"groundwork","default_model":"ea-groundwork","posthoc_review_profile":"review_light","fallback_brain_profile":"survival","output_mode":"json"},"provider_hints_json":{"primary":["1min.AI"],"research":["BrowserAct"],"output":["1min.AI","Media Factory","AI Magicx","Prompting Systems","BrowserAct","Gemini Vortex"],"media":["Media Factory","AI Magicx","Prompting Systems","BrowserAct"],"style":["1min.AI","Gemini Vortex"]},"tool_policy_json":{"allowed_tools":["provider.brain_router.structured_generate","artifact_repository"]},"human_policy_json":{"review_roles":["guide_reviewer"]},"evaluation_cases_json":[{"case_key":"chummer6_guide_refresh_golden","priority":"medium"}],"budget_policy_json":{"class":"low","workflow_template":"tool_then_artifact","pre_artifact_capability_key":"structured_generate","artifact_failure_strategy":"retry","artifact_max_attempts":2,"artifact_retry_backoff_seconds":1,"style_epoch_enabled":true,"variation_guard_enabled":true}}')"
...
if [[ "${CHUMMER_SKILL_FIELDS}" != "chummer6_visual_director|chummer6_guide_refresh|tool_then_artifact|1min.AI|chummer6_guide_refresh|groundwork|chummer6_visual_director|3|step_input_prepare,step_structured_generate,step_artifact_save|provider.brain_router.structured_generate" ]]; then
  echo "expected chummer6_visual_director to compile through the EA groundwork structured-generation lane; got ${CHUMMER_SKILL_FIELDS}" >&2
...
path = Path(os.environ["SMOKE_TMP_DIR"]) / "ea_memory_mismatch_resp.json"
if not path.exists():
    print("")
    raise SystemExit(0)
try:
