...
SKILL_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.argv[1]); listed=json.loads(sys.argv[2]); fetched=json.loads(sys.argv[3]); compiled=json.loads(sys.argv[4]); steps=compiled.get('plan',{}).get('steps') or []; print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('skill_key',''), body.get('workflow_template',''), ','.join(body.get('memory_reads') or []), ','.join(body.get('memory_writes') or []), ','.join((body.get('provider_hints_json') or {}).get('primary') or []), any(row.get('skill_key') == 'meeting_prep' for row in listed), fetched.get('name',''), (fetched.get('authority_profile_json') or {}).get('authority_class',''), ','.join((fetched.get('provider_hints_json') or {}).get('research') or []), compiled.get('skill_key',''), len(steps), ','.join(step.get('step_key','') for step in steps)))" "${SKILL_JSON}" "${SKILL_LIST_JSON}" "${SKILL_FETCH_JSON}" "${SKILL_PLAN_JSON}")"
if [[ "${SKILL_FIELDS}" != "meeting_prep|artifact_then_memory_candidate|stakeholders,commitments,decision_windows|meeting_pack_fact|1min.AI|True|Meeting Prep|draft|BrowserAct,Paperguide|meeting_prep|4|step_input_prepare,step_policy_evaluate,step_artifact_save,step_memory_candidate_stage" ]]; then
