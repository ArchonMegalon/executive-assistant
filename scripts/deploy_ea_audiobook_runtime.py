...
stopped worker stage after validating the production-stage evidence; it never
starts or activates that stage. No reachable command builds, pulls, uploads,
sends a message, calls a provider, or changes ingress/webhooks.
"""
...
DEPLOY_RECEIPT_CONTRACT = "ea.audiobook_runtime_governed_deploy.v2"
...
PRODUCTION_PREFLIGHT_CONTRACT = "ea.audiobook_runtime_production_preflight.v2"
PRODUCTION_PREFLIGHT_VERSION = 2
PRODUCTION_PROJECTION_CONTRACT = "ea.audiobook_runtime_production_projection.v2"
PRODUCTION_PROJECTION_VERSION = 2
...
class AudiobookRuntimeDeployLane:
    ...
    def _guard_now(self) -> datetime:
        try:
            ...
            raise DeployError("deployment_clock_invalid") from exc
        if (
            not isinstance(now, datetime)
            or now.tzinfo is None
            or now.utcoffset() != UTC.utcoffset(now)
        ):
            raise DeployError("deployment_clock_invalid")
    ...
    def _validate_production_projection(
        ...
        if (
            ...
            or configuration.get("version") != PRODUCTION_PREFLIGHT_VERSION
            ...
            != "governed_consumer_may_create_paused_stage_after_live_preflight"
        ):
        ...
        if (
            ...
            or projection.get("version") != PRODUCTION_PROJECTION_VERSION
    ...
    def _revalidate_evidence(self, boundary: str) -> None:
        if not all(
            (
            ...
            raise DeployError("deployment_evidence_missing")
        ...
        self.receipt["checks"] = [
            *list(self.receipt.get("checks") or []),
            {"name": "evidence_revalidation", "status": "pass", "boundary": boundary},
        ]

    @contextmanager
    def _live_evidence_guard(self) -> Iterator[None]:
        self._revalidate_evidence("immediately_before:paused_stage_transaction")
        yield
    ...
    def _protect_previous_images(
        ...
        deployment_digest = hashlib.sha256(self.deployment_id.encode("utf-8")).hexdigest()
        for service in services:
            if (
                self.pre_state["services"][service]["runtime"].get(
                    "lifecycle", "running"
            ...
            tag = (
                ...
                f"{self.deployment_id[:32]}-{deployment_digest[:12]}-{image_id[-12:]}"
            )
    ...
    def _execute_paused_stage_transaction(self) -> None:
        ...
        with self._live_evidence_guard():
            ...
            self._revalidate_evidence("before:paused_stage_create")
            self._revalidate_candidate_image()
            self._revalidate_target_configuration()
            self._revalidate_pre_state(EXPECTED_RUNTIME_SERVICES)
            self._verify_controls(self.memorial_controls)
            ...
            try:
                ...
                self._verify_controls(self.memorial_controls)
                self._revalidate_evidence("immediately_after:paused_stage_create")
                self._verify_paused_stage(WORKER_SERVICES, self.forward_compose_files)
                self._revalidate_pre_state(PRESERVED_RUNTIME_SERVICES)
                self._verify_controls(self.memorial_controls)
    ...
    def deploy(self, *, execute: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            self.preflight()
            if not execute:
                self.receipt["status"] = "preflight_only"
                self.receipt["completed_at"] = _utc_text(self.utc_now())
                self._write_receipt()
                return self.receipt
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Create the exact worker stage without starting it after validating "
            "the production-stage evidence. Without this flag the command is "
            "preflight-only."
        ),
    )
    parser.add_argument(
