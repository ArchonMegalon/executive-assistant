...
is non-transferable validation input for the governed deployment consumer.
"""
...
ROOT_CONTRACT = "ea.audiobook_runtime_production_preflight.v2"
ROOT_CONTRACT_VERSION = 2
PROJECTION_CONTRACT = "ea.audiobook_runtime_production_projection.v2"
...
def _open_absolute_nofollow(
    ...
    *,
    ...
    if not path.is_absolute() or path.name in {"", ".", ".."}:
        raise ProductionStageError(f"{reason}_location_invalid")
    required = ("O_NOFOLLOW", "O_NONBLOCK", "O_DIRECTORY")
    if any(not hasattr(os, name) for name in required):
        raise ProductionStageError(f"{reason}_safe_open_unavailable")
    ...
    try:
        ...
        if require_root_parents:
            ...
            if not _root_anchor_is_trusted(root_metadata, current_root):
                raise ProductionStageError(f"{reason}_root_anchor_untrusted")
        elif not stat.S_ISDIR(root_metadata.st_mode):
            raise ProductionStageError(f"{reason}_parent_untrusted")
