...
This bounded controller deliberately has no Docker mutation primitive. It records
two-sample identity observations for registered v5 candidates and quarantines
legacy, invalid, unstable, and unregistered postures. It never selects a retained
candidate or grants retirement authority.
"""
...
REPORT_SCHEMA = "ea.manfred_memorial_candidate_retention.v3"
    ...
    *,
    ...
    for raw_posture in postures:
        ...
        if project == LIVE_COMPOSE_PROJECT:
            ...
            continue
        if schema in {RUNTIME_SCHEMA_V3, RUNTIME_SCHEMA_V4}:
            candidates[project] = _quarantine_row(
                project,
            ...
            continue
        if schema != RUNTIME_SCHEMA_V5 or posture.get("legacy") is not False:
            candidates[project] = _quarantine_row(
                project,
                runtime_schema=schema or "unknown",
    ...
    report.update(
        {
            ...
            "identity_stable_projects": sorted(stable_projects),
            "candidates": rows,
