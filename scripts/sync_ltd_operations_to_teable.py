...
try:
    ...
    from bootstrap_proactive_ooda_teable_tables import (  # type: ignore[import-not-found, no-redef]
        DEFAULT_BASE_URL,
        _api_key,
        _create_table,
        _discover_table_id,
...
CAPACITY_CONFIG = ROOT / "config/ltd_capacity_scheduler.yaml"
LTD_INVENTORY = ROOT / "LTDs.md"
PAGE_SIZE = 1000
...
TABLES: dict[str, list[dict[str, object]]] = {
    "ltd_provider_status": [
        ...
        _text("route_state"),
        _text("credit_balance_state"),
        _text("slot_health_state"),
        _checkbox("dispatch_eligible"),
        _text("source_status"),
        _text("source_sha256"),
...
def _source_is_current(payload: dict[str, Any], *, digest_field: str, source_path: Path) -> bool:
    expected = str(payload.get(digest_field) or "").strip().lower()
    return bool(
        source_path.is_file()
        and re.fullmatch(r"[0-9a-f]{64}", expected)
        and expected == _sha256_file(source_path)
    )
...
def build_rows(*, provider_status_path: Path, proof_debt_path: Path) -> dict[str, list[dict[str, object]]]:
    ...
    for name, payload in (("provider", provider_payload), ("proof_debt", proof_payload)):
        if bool(payload.get("secret_material_exposed")):
            raise SystemExit(f"ltd_{name}_projection_exposes_secret_material")
    if provider_payload.get("contract") != "ea.ltd_capacity_status.v1":
        raise SystemExit("ltd_provider_projection_contract_invalid")
    if proof_payload.get("contract") != "ea.ltd_proof_debt.v1":
        raise SystemExit("ltd_proof_debt_projection_contract_invalid")

    provider_current = _source_is_current(
        provider_payload,
        digest_field="config_sha256",
        source_path=CAPACITY_CONFIG,
    )
    proof_current = _source_is_current(
        proof_payload,
        digest_field="source_sha256",
        source_path=LTD_INVENTORY,
    )
        ...
        provider_rows.append(
            {
                ...
                "credit_balance_state": str(source.get("credit_balance_state") or "unverified"),
                "slot_health_state": str(source.get("slot_health_state") or "unverified"),
                "dispatch_eligible": bool(source.get("dispatch_eligible")),
                ...
                "current_or_stale": "current" if provider_current else "stale",
                "privacy_secret_material_exposed": bool(),
            }
        )
        ...
        proof_rows.append(
            {
                ...
                "current_or_stale": "current" if proof_current else "stale",
                "privacy_secret_material_exposed": bool(),
            }
        )
...
def _list_records(*, base_url: str, api_key: str, table_id: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if isinstance(payload, dict):
            source = payload.get("records") or payload.get("data") or []
        else:
            source = payload
...
def _upsert_rows(
    *, base_url: str, api_key: str, table_id: str, rows: list[dict[str, object]]
    ...
    for row in rows:
        ...
        existing_record = existing.get(projection_id)
        if not existing_record:
            creates.append({"fields": row})
            continue
        record_id = str(existing_record.get("id") or "").strip()
        current_fields = dict(existing_record.get("fields") or {})
...
def main() -> int:
    ...
    stale_source_count = sum(
        ...
        for row in rows
        if row.get("current_or_stale") != "current"
    )
    ...
    if not args.apply:
        ...
        return 0 if not stale_source_count else 1

    if stale_source_count:
        raise SystemExit("ltd_projection_sources_stale")
