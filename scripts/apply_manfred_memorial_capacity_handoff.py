...
HANDOFF_SCHEMA = "ea.manfred_memorial_build_capacity.root_handoff.v2"
...
DELETION_JOURNAL_SCHEMA = "ea.manfred_memorial_build_capacity.deletion_journal.v2"
DELETION_COMPLETE_SCHEMA = "ea.manfred_memorial_build_capacity.deletion_complete.v2"
...
TARGET_ROOT_FREE_BYTES = 35 * 1024**3 + 256 * 1024**2
    ...
    if not os.path.lexists(logical_path):
        ...
        if raw.get("kind") != "candidate_release_projection":
            journal_path = recovery_root / (
                f"{_candidate_quarantine_name(raw)}.journal.v2.json"
            )
            ...
            complete_path = recovery_root / (
                f"{_candidate_quarantine_name(raw)}.complete.v2.json"
            )
...
def _remove_generic_candidate(
    ...
    *,
    ...
    journal_path = quarantine_root / f"{quarantine_name}.journal.v2.json"
    complete_path = quarantine_root / f"{quarantine_name}.complete.v2.json"
