...
This module has no Docker, Compose, HTTP, permit, or cleanup capability.
"""
...
BUNDLE_CONTRACT = "ea.memorial_api_baseline_bundle.v1"
BUNDLE_VERSION = 1
...
def _read_at(
    ...
    *,
    ...
    try:
        ...
        if total != before.st_size or _identity(after) != identity or _identity(final) != identity:
            raise BaselineBundleError(reason + "_changed")
        raw = b"".join(chunks)
        return raw, {
            "git_blob_id": None,
...
def _repository_descriptor_cwd(descriptor: int) -> Path:
    ...
    try:
        ...
        raise BaselineBundleError("repository_descriptor_projection_unavailable") from exc
    if (
        not stat.S_ISDIR(opened.st_mode)
...
def _git_line(
    runner: Runner, root: Path, args: Sequence[str], reason: str
) -> str:
...
def _validate_compose_tree(root: Node, allowed_tagged_nodes: set[int]) -> None:
    ...
    def walk(node: Node) -> None:
        ...
        if not node.tag.startswith("tag:yaml.org,2002:"):
            if (
                node_id not in allowed_tagged_nodes
                or node.tag not in {"!reset", "!override"}
            ):
...
def _compose_environment_names(raw: bytes) -> tuple[set[str], bool]:
    ...
    try:
        documents = list(
            yaml.compose_all(raw.decode("utf-8"), Loader=yaml.SafeLoader)
        )
    ...
    if (
        not isinstance(services, MappingNode)
        or services.tag != "tag:yaml.org,2002:map"
    ):
    ...
    for service_key, service_value in services.value:
        ...
        if isinstance(service_value, MappingNode):
            if _mapping_value(
                service_value, "extends", "compose_extends_ambiguous"
            ) is not None:
                raise BaselineBundleError("compose_extends_unsupported")
    api = _mapping_value(services, "ea-api", "compose_api_ambiguous")
    if (
        not isinstance(api, MappingNode)
        or api.tag != "tag:yaml.org,2002:map"
    ):
        raise BaselineBundleError("compose_api_invalid")
    environment = _mapping_value(
        api, "environment", "compose_environment_ambiguous"
    )
    ...
    if isinstance(environment, SequenceNode):
        ...
        for item in environment.value:
            if (
                ...
                or item.tag != "tag:yaml.org,2002:str"
            ):
        ...
        if quote == '"' and character == "\\":
            ...
            if index >= len(value) or value[index] not in {
                ...
                raise BaselineBundleError(
                    "trusted_environment_quoting_invalid"
                )
...
def _override(names: Sequence[str]) -> bytes:
    ...
    if names:
        ...
        dollar = chr(36)
        lines.extend(
            f"      - {name}={dollar}{{{name}}}" for name in names
        )
    return ("\n".join(lines) + "\n").encode("utf-8")
...
def _validate_bundle(path: Path) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        if (
            ...
            or not REVISION_RE.fullmatch(
                str(manifest.get("origin_main_commit"))
            )
            ...
            or not SHA_RE.fullmatch(
                str(manifest.get("environment_key_set_sha256"))
            )
            ...
            or not all(
                _record_valid(item, allow_absent=True) for item in environment
            )
        ):
...
def _require_bundle_semantics(
    ...
    *,
    ...
    try:
        ...
        local_present = environment_records[1]["present"] is True
        retained_local, retained_local_record = _read_at(
            directory_fd,
            ".env.local",
            required=local_present,
            reason="retained_env_local",
        )
        if not local_present:
            if retained_local is not None:
                raise BaselineBundleError(
                    "existing_bundle_environment_presence_mismatch"
                )
            _absent_at(
                directory_fd, ".env.local", "retained_env_local"
            )
        ...
        for current, recorded in zip(
            ...
            if current != recorded:
                raise BaselineBundleError(
                    "existing_bundle_environment_record_mismatch"
                )

        semantic_env = retained_env
        semantic_local = retained_local
        if authoritative_environment is not None:
            ...
            if (
                ...
                or retained_local != trusted_local
                or len(trusted_records) != 2
                or any(
                    dict(current) != recorded
                    for current, recorded in zip(
                        trusted_records, environment_records, strict=True
                    )
                )
            ):
                raise BaselineBundleError(
            ...
            semantic_env = trusted_env
            semantic_local = trusted_local

        trusted_names = _env_names(
            [
                semantic_env,
                *([semantic_local] if semantic_local is not None else []),
            ]
        )
        env_only = sorted(trusted_names - _explicit_environment_names(blobs))
...
def _sealed_retained_blobs(
    ...
    try:
        ...
        for index, relative in enumerate(COMPOSE_BLOB_PATHS):
            ...
            assert raw is not None
            object_id = str(
                manifest["ordered_compose_files"][index]["git_blob_id"]
            )
            if (
...
def _require_recovery_seal(
    ...
    if (
        ...
        or (
            ...
            and recovery_seal.get("plan_sha256") != plan_sha
        )
    ):
...
def _materialize_baseline_bundle(
    *,
    ...
    if test_hooks is not None and test_hooks.token is not _TEST_HOOKS_TOKEN:
        raise BaselineBundleError("test_hooks_authority_invalid")
    durable_root_check = (
        _default_durable_check
        if test_hooks is None
        else test_hooks.durable_root_check
    )
    ...
    name = "api-baseline-" + str(plan["plan_id"])
    ...
    try:
        ...
        env_raw, local_raw, trusted_environment_records = (
            _read_trusted_environment(trusted_root)
        )
        trusted_names = _env_names(
            [env_raw, *([local_raw] if local_raw is not None else [])]
        )
        env_only = sorted(trusted_names - _explicit_environment_names(blobs))
        ...
        try:
            ...
            if local_raw is not None:
                copied_environment_records.append(
                    _write_at(bundle_fd, ".env.local", local_raw)
                )
            else:
                copied_environment_records.append(
                    {
            ...
            if copied_environment_records != trusted_environment_records:
