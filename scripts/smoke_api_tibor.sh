...
CONNECTOR_JSON="$(operator_post_json "${BASE}/v1/connectors/bindings" -H 'content-type: application/json' \
...
if [[ -n "${BINDING_ID}" ]]; then
  ...
  operator_post_json "${BASE}/v1/connectors/bindings/${BINDING_ID}/status" -H 'content-type: application/json' -d '{"status":"disabled"}' >/dev/null
fi
operator_curl "${BASE}/v1/connectors/bindings?limit=5" >/dev/null
CONNECTOR_MISMATCH_CODE="$(curl_status_code /tmp/tibor_ea_connector_mismatch_resp.json "${BASE}/v1/connectors/bindings?principal_id=${PRINCIPAL_ID}&limit=5" "${AUTH_ARGS[@]}" -H "X-EA-Principal-ID: ${MISMATCH_PRINCIPAL_ID}")"
