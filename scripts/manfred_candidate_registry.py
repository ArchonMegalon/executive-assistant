...
LEGACY_RUNTIME_SCHEMAS = frozenset({RUNTIME_SCHEMA_V3, RUNTIME_SCHEMA_V4})
SUPPORTED_RUNTIME_SCHEMAS = LEGACY_RUNTIME_SCHEMAS | {RUNTIME_SCHEMA_V5}
...
def _runtime_identity(payload: dict[str, object]) -> dict[str, object]:
    ...
    if schema == RUNTIME_SCHEMA_V5:
        if payload.get("candidate_left_running") is not True:
            raise RuntimeError("manfred_candidate_registry_receipt_invalid")
    ...
    return {
        ...
        "revision": revision,
        "port": port,
        "legacy": schema in LEGACY_RUNTIME_SCHEMAS,
    }
...
def register_candidate_receipt(
    ...
    *,
    ...
    if identity["schema"] != RUNTIME_SCHEMA_V5:
        ...
        postures.append(
            {
                **entry,
                ...
                "quarantined": legacy,
                "quarantine_reason": (
                    f"legacy_runtime_receipt_{str(identity['schema']).rsplit('.', 1)[-1]}"
                    if legacy
                    else ""
                ),
