...
This lane deliberately performs no mutation.  The existing root-owned memorial
permit authorizes API/Redis boundaries only; it does not authorize a
``ea-cloudflared`` recreation.  The script therefore captures and validates the
inputs a future joint coordinator needs, proves the public edge when requested,
and denies standalone reconciliation before any mutating command can run.
"""
...
try:
    from scripts.deploy_ea_memorial import (
        ...
        DEFAULT_VEXP_MUTATION_PERMIT_PATH,
        MEMORIAL_SLUG,
        PROJECT_NAME,
...
class PublicIngressReconciliationLane(MemorialDeployLane):
    ...
    def __init__(
        ...
        *,
        ...
        self.receipt = {
            "contract_name": "ea.public_ingress_reconciliation_preflight.v1",
            ...
            "coordinator": {
                "status": "deny",
                "reason": "standalone_cloudflared_mutation_not_permitted",
    ...
    def preflight(self) -> dict[str, Any]:
        ...
        self.receipt["permit_contract"] = {
            "contract_name": VEXP_MUTATION_PERMIT_CONTRACT_NAME,
            "version": VEXP_MUTATION_PERMIT_VERSION,
            "path": str(DEFAULT_VEXP_MUTATION_PERMIT_PATH),
            "authorized_boundaries": list(VEXP_MUTATION_BOUNDARIES),
            "ingress_boundary_authorized": False,
        }
    ...
    def run(
        ...
        try:
            ...
            self.receipt["coordinator"] = {
                "status": "deny",
                "reason": "no_exact_root_permit_boundary_for_cloudflared",
                "joint_api_ingress_atomicity_proven": False,
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "joint root-permitted coordinator exists."
        )
    )
