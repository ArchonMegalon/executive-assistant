...
VEXP_BIN = shutil.which("vexp") or "vexp"
QUERIES = (
...
def _head() -> str:
    result = subprocess.run(  # nosec B603
        [GIT_BIN, "rev-parse", "HEAD"], cwd=ROOT, check=True, capture_output=True, text=True
    )
...
def _run_query(query: str, *, timeout_seconds: int) -> dict[str, object]:
    try:
        completed = subprocess.run(  # nosec B603
            [VEXP_BIN, "capsule", "--format", "json", "--max-tokens", "1200", query],
            cwd=ROOT,
            capture_output=True,
            text=True,
