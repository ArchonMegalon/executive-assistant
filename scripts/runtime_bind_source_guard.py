#!/usr/bin/env python3
"""Fail-closed access checks for bind sources used by the runtime API.
...
RUNTIME_CARTESIA_CREDENTIAL_TARGET = "/run/secrets/ea_runtime_cartesia.json"
MAX_RUNTIME_CARTESIA_CREDENTIAL_BYTES = 64 * 1024
...
def _parse_numeric_identity(value: object) -> tuple[int, int]:
    ...
    if (
        ...
        raise BindSourceGuardError("runtime_api_numeric_user_required")
    uid, gid = (int(piece) for piece in pieces)
    if not 1 <= uid <= 2**31 - 1 or not 1 <= gid <= 2**31 - 1:
        raise BindSourceGuardError("runtime_api_numeric_user_invalid")
    return uid, gid
...
def _supplemental_gids(value: object, *, primary_gid: int) -> frozenset[int]:
    if value is None:
        ...
        raise BindSourceGuardError("runtime_api_group_add_invalid")
    gids = {primary_gid}
    for row in rows:
        normalized = str(row or "").strip()
        if not normalized.isdigit() or str(int(normalized)) != normalized:
            raise BindSourceGuardError("runtime_api_group_add_must_be_numeric")
        gid = int(normalized)
        if not 1 <= gid <= 2**31 - 1:
            raise BindSourceGuardError("runtime_api_group_add_invalid")
        gids.add(gid)
...
def _require_cartesia_credential_source(
    ...
    *,
    ...
    if not 1 <= metadata.st_size <= MAX_RUNTIME_CARTESIA_CREDENTIAL_BYTES:
...
def _validate_runtime_bind_sources(
    ...
    *,
    ...
    if not isinstance(services, dict) or not isinstance(services.get(service), dict):
        raise BindSourceGuardError("runtime_api_service_missing")
    config = dict(services[service])
    if str(config.get("user") or "").strip() != expected_user:
        raise BindSourceGuardError("runtime_api_explicit_user_mismatch")
    ...
    if not isinstance(volumes, list):
        raise BindSourceGuardError("runtime_api_volumes_invalid")
    if (
    ...
    for raw_mount in volumes:
        if not isinstance(raw_mount, dict):
            raise BindSourceGuardError("runtime_api_mount_invalid")
        ...
        if (
            ...
            raise BindSourceGuardError("runtime_api_bind_mount_invalid")
        if "read_only" not in raw_mount:
            ...
            raise BindSourceGuardError("runtime_api_bind_read_only_invalid")
        targets.add(target)
        ...
        try:
            ...
            if target == RUNTIME_CARTESIA_CREDENTIAL_TARGET:
    ...
    if not mount_evidence:
        raise BindSourceGuardError("runtime_api_bind_mounts_missing")
